 ...
 * Mat4x4.h                                                                 *
 *                                                                          *
 * Mat4x4 is a 4x4 matrix class, with associated operators.  If "M" is an   *
 * instance of the class Mat4x4, then the ij'th element of M is accessed    *
 * as M(i,j), where i and j are ints between 0 and 3, inclusive.  M(i,j)    *
 * is both a "const" reference and a reference; that is, one can read the   *
 * the elements of M if it is passed in as a const reference, and also      *
 * assign values to the elements if it is non-const.                        *
 *                                                                          *
 *                                                                          *
 ***************************************************************************/
...
public:
    inline Mat4x4();
    inline Mat4x4( const Mat4x4 &M ) { *this = M; }
    ...
    inline void SetTrans( const Vec2 &v ) { m[0][3] = v.x; m[1][3] = v.y; }
    inline void SetTrans( const Vec3 &v ) { m[0][3] = v.x; m[1][3] = v.y; m[2][3] = v.z; }
